...
version: 1.0.0
author: Galaxy Maps
repository: https://github.com/Galaxy-Maps/gm-agent-03a-curriculum-with-agent-teams
standalone: true
mode: agent-team
...
| Tool | Purpose |
|------|---------|
| `Read` | Read INTENT.md, plus each team's `teams/{team-name}.md` prompt file |
| `Write` | Write each team's consensus `MAP_V1_{TEAM}.md` and the final `MAP_V1.md` |
| `Bash` | `git add MAP_V1_*.md MAP_V1.md && git commit` |
| `TeamCreate` / `TeamDelete` | Open/close one Agent Team per selected curriculum-design team |
| `Task` | Spawn each theorist as a teammate (`team_name`, `subagent_type: "general-purpose"`) |
| `SendMessage` | Teammates debate via peer messaging; lead receives convergence signal |
| `AskUserQuestion` | Team selection menu (one / multiple / all), and final MAP selection when multiple teams ran |

---

## Standalone Usage

When invoked by the orchestrator (`gm-agent-01a-orchestrator-with-agent-teams`), the orchestrator handles the lifecycle calls below. When invoked directly (e.g. `Skill({ skill: "gm-agent-03a-curriculum-with-agent-teams" })`), this skill must run the team itself:

1. Ask the user to choose one, multiple, or all 7 teams (see **Team Selection** below).
2. For each selected team, in sequence (Claude Code currently supports one Agent Team per session):
   - `Read` the team's prompt file at `teams/{team-name}.md`.
   - `TeamCreate({ team_name: "{team-name}-team" })`.
   - Spawn each theorist with `Task({ name, team_name, subagent_type: "general-purpose", prompt: <shared context + persona> })`.
   - Wait for the team to converge. Teammates debate via `SendMessage`.
   - Compile `MAP_V1_{TEAM_NAME}.md` from the agreed structure.
   - `TeamDelete()` before starting the next team.
3. If multiple teams ran, present all `MAP_V1_*.md` files and ask the user to pick one (`AskUserQuestion`).
4. Copy the selected map to `MAP_V1.md`.
5. `git add MAP_V1_*.md MAP_V1.md && git commit -m "feat(curriculum): add curriculum structure v1 ({team-name} team)"`.
